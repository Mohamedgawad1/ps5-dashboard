(() => {
  ...
  function updateCards(display){
    const closed = display.closed||0, eit = display.eit_total||0;
    ...
    document.querySelectorAll('.kpi').forEach(c=>{
      ...
      if(/CPP AGI EIT/i.test(t) && !/Today/i.test(t)){
        v.textContent = fmt(closed) + (eit>0 ? ' / '+fmt(eit) : '');
      } else if(/Progress/i.test(t)){
        v.textContent = pctTxt + '%';
      }
    });
  }
  function build2(update){
    if(update && window.__itrKillCards !== true){ try{ updateCards(update); }catch(e){} }
